...
int main (int argc, char* argv[]) {
  ...
  BOOST_LOG_TRIVIAL(info) << "==========Baby MIND background==========";
  ...
  TH1D *hist_bm_bg_pi_mom = new TH1D("hist_bm_bg_pi_mom", ";Reconstructed momentum [MeV/c];Entries", 50, 0., 1500.);
  TH1D *hist_bm_bg_pi_mom_mcs = new TH1D("hist_bm_bg_pi_mom_mcs", ";Reconstructed momentum [MeV/c];Entries", 50, 0., 1500.);
  TH1D *hist_bm_bg_pi_mom_range = new TH1D("hist_bm_bg_pi_mom_range", ";Reconstructed momentum [MeV/c];Entries",
					   50, 0., 1500.);
  TH1D *hist_bm_bg_p_mom = new TH1D("hist_bm_bg_p_mom", ";Reconstructed momentum [MeV/c];Entries", 50, 0., 1500.);
  TH1D *hist_bm_bg_p_mom_mcs = new TH1D("hist_bm_bg_p_mom_mcs", ";Reconstructed momentum [MeV/c];Entries", 50, 0., 1500.);
  TH1D *hist_bm_bg_p_mom_range = new TH1D("hist_bm_bg_p_mom_range", ";Reconstructed momentum [MeV/c];Entries",
					  50, 0., 1500.);
  TH1D *hist_bm_bg_pi_cos = new TH1D("hist_bm_bg_pi_cos", ";Reconstructed angle;Entries", 40, -1., 1.);
  TH1D *hist_bm_bg_pi_deg = new TH1D("hist_bm_bg_pi_deg", ";Reconstructed angle [degree];Entries", 36, 0., 180.);
  TH1D *hist_bm_bg_p_cos = new TH1D("hist_bm_bg_p_cos", ";Reconstructed angle;Entries", 40, -1., 1.);
  TH1D *hist_bm_bg_p_deg = new TH1D("hist_bm_bg_p_deg", ";Reconstructed angle [degree];Entries", 36, 0., 180.);
  ...
  for ( auto ev : ev_vec ) {
	...
	if ( chain.particle_flag % 10000 == 211 ) {
	  if ( chain.stop_flag == 0 ) {
	    ...
	    hist_bm_bg_pi_mom->Fill(momentum, ev.weight);
	    hist_bm_bg_pi_mom_mcs->Fill(momentum, ev.weight);
	  }
	  else if ( chain.stop_flag == 2 ) {
	    momentum = chain.ecc_range_mom[0];
	    hist_bm_bg_pi_mom->Fill(momentum, ev.weight);
	    hist_bm_bg_pi_mom_range->Fill(momentum, ev.weight);
	  }
	  hist_bm_bg_pi_cos->Fill(cosine, ev.weight);
	  hist_bm_bg_pi_deg->Fill(theta_deg, ev.weight);
	}
	else if ( chain.particle_flag % 10000 == 2212 ) {
	  if ( chain.stop_flag == 0 ) {
	    momentum = chain.ecc_mcs_mom[1];
	    hist_bm_bg_p_mom->Fill(momentum, ev.weight);
	    hist_bm_bg_p_mom_mcs->Fill(momentum, ev.weight);
	  }
	  else if ( chain.stop_flag == 2 ) {
	    momentum = chain.ecc_range_mom[1];
	    hist_bm_bg_p_mom->Fill(momentum, ev.weight);
	    hist_bm_bg_p_mom_range->Fill(momentum, ev.weight);
	  }
	  hist_bm_bg_p_cos->Fill(cosine, ev.weight);
	  hist_bm_bg_p_deg->Fill(theta_deg, ev.weight);
	}
      }
    }
  ...
  hist_bm_bg_pi_mom->Write();
  hist_bm_bg_pi_mom_mcs->Write();
  hist_bm_bg_pi_mom_range->Write();
  hist_bm_bg_p_mom->Write();
  hist_bm_bg_p_mom_mcs->Write();
  hist_bm_bg_p_mom_range->Write();
  hist_bm_bg_pi_cos->Write();
  hist_bm_bg_pi_deg->Write();
  hist_bm_bg_p_cos->Write();
  hist_bm_bg_p_deg->Write();
