...
int main (int argc, char* argv[]) {
  ...
  TH1D *hist_pm_bg_mu_mom = new TH1D("hist_pm_bg_mu_mom", "", muon_mom_bin_size - 1, muon_mom_bins);
  TH1D *hist_pm_bg_mu_mom_mcs = new TH1D("hist_pm_bg_mu_mom_mcs", "", muon_mom_bin_size - 1, muon_mom_bins);
  TH1D *hist_pm_bg_mu_mom_range = new TH1D("hist_pm_bg_mu_mom_range", "", muon_mom_bin_size - 1, muon_mom_bins);

  TH1D *hist_pm_bg_mu_mom_mcs_all = new TH1D("hist_pm_bg_mu_mom_mcs_all", "", muon_mom_bin_size - 1, muon_mom_bins);

  TH1D *hist_pm_bg_mu_mom_true = new TH1D("hist_pm_bg_mu_mom_true", "", muon_mom_bin_size - 1, muon_mom_bins);
  TH1D *hist_pm_bg_mu_mom_true_stop = new TH1D("hist_pm_bg_mu_mom_true_stop", "", muon_mom_bin_size - 1, muon_mom_bins);
  TH1D *hist_pm_bg_mu_mom_true_nonstop = new TH1D("hist_pm_bg_mu_mom_true_nonstop", "", muon_mom_bin_size - 1, muon_mom_bins);  

  TH1D *hist_pm_bg_mu_cos = new TH1D("hist_pm_bg_mu_cos", "", muon_cos_bin_size - 1, muon_cos_bins);
  TH1D *hist_pm_bg_mu_deg = new TH1D("hist_pm_bg_mu_deg", "", muon_deg_bin_size - 1, muon_deg_bins);

  TH1D *hist_pm_bg_pi_mom = new TH1D("hist_pm_bg_pi_mom", ";Reconstructed momentum [MeV/c];Entries", hadron_mom_bin_size - 1, hadron_mom_bins);
  TH1D *hist_pm_bg_pi_mom_mcs = new TH1D("hist_pm_bg_pi_mom_mcs", ";Reconstructed momentum [MeV/c];Entries", hadron_mom_bin_size - 1, hadron_mom_bins);
  TH1D *hist_pm_bg_pi_mom_range = new TH1D("hist_pm_bg_pi_mom_range", ";Reconstructed momentum [MeV/c];Entries",
					   hadron_mom_bin_size - 1, hadron_mom_bins);
  TH1D *hist_pm_bg_p_mom = new TH1D("hist_pm_bg_p_mom", ";Reconstructed momentum [MeV/c];Entries", hadron_mom_bin_size - 1, hadron_mom_bins);
  TH1D *hist_pm_bg_p_mom_mcs = new TH1D("hist_pm_bg_p_mom_mcs", ";Reconstructed momentum [MeV/c];Entries", hadron_mom_bin_size - 1, hadron_mom_bins);
  TH1D *hist_pm_bg_p_mom_range = new TH1D("hist_pm_bg_p_mom_range", ";Reconstructed momentum [MeV/c];Entries",
					  hadron_mom_bin_size - 1, hadron_mom_bins);
  TH1D *hist_pm_bg_pi_cos = new TH1D("hist_pm_bg_pi_cos", ";Reconstructed angle;Entries", hadron_cos_bin_size - 1, hadron_cos_bins);
  TH1D *hist_pm_bg_pi_deg = new TH1D("hist_pm_bg_pi_deg", ";Reconstructed angle [degree];Entries", hadron_deg_bin_size - 1, hadron_deg_bins);
  TH1D *hist_pm_bg_p_cos = new TH1D("hist_pm_bg_p_cos", ";Reconstructed angle;Entries", hadron_cos_bin_size - 1, hadron_cos_bins);
  TH1D *hist_pm_bg_p_deg = new TH1D("hist_pm_bg_p_deg", ";Reconstructed angle [degree];Entries", hadron_deg_bin_size - 1, hadron_deg_bins);

  TH1D *hist_pm_bg_other_mom_mcs = new TH1D("hist_pm_bg_other_mom_mcs", "", hadron_mom_bin_size - 1, hadron_mom_bins);
  TH1D *hist_pm_bg_other_cos = new TH1D("hist_pm_bg_other_cos", "", hadron_cos_bin_size - 1, hadron_cos_bins);
  TH1D *hist_pm_bg_other_deg = new TH1D("hist_pm_bg_other_deg", "", hadron_deg_bin_size - 1, hadron_deg_bins);
  ...
  for ( auto ev : ev_vec ) {
	...
	if ( recon_particle_id == 13 ) {
	  ...
	  if ( chain.stop_flag == 1 ) {
	    ...
	    hist_pm_bg_mu_mom_range->Fill(momentum, ev.weight);
	    hist_pm_bg_mu_mom_true_stop->Fill(mu_mom_true, ev.weight);
	  }
	  else if ( chain.stop_flag == 0 ) {
	    momentum = chain.ecc_mcs_mom[0];
	    hist_pm_bg_mu_mom_mcs->Fill(momentum, ev.weight);
	    hist_pm_bg_mu_mom_true_nonstop->Fill(mu_mom_true, ev.weight);
	  }
	  hist_pm_bg_mu_mom->Fill(momentum, ev.weight);
	  hist_pm_bg_mu_deg->Fill(theta_deg, ev.weight);
	  hist_pm_bg_mu_cos->Fill(cosine, ev.weight);
	  
	  hist_pm_bg_mu_mom_mcs_all->Fill(chain.ecc_mcs_mom[0], ev.weight);

	  hist_pm_bg_mu_mom_true->Fill(mu_mom_true, ev.weight);
	}
	else if ( recon_particle_id == 2212 ) {
	  if ( chain.stop_flag == 0 ) {
	    momentum = chain.ecc_mcs_mom[1];
	    hist_pm_bg_p_mom_mcs->Fill(momentum, ev.weight);
	  }
	  else if ( chain.stop_flag == 2 ) {
	    momentum = chain.ecc_range_mom[1];
	    hist_pm_bg_p_mom_range->Fill(momentum, ev.weight);
	  }
	  hist_pm_bg_p_mom->Fill(momentum, ev.weight);
	  hist_pm_bg_p_cos->Fill(cosine, ev.weight);
	  hist_pm_bg_p_deg->Fill(theta_deg, ev.weight);
	}
	else if ( recon_particle_id == 211 ) {
	  if ( chain.stop_flag == 0 ) {
	    momentum = chain.bm_curvature_mom;
	    hist_pm_bg_pi_mom->Fill(momentum, ev.weight);
	    hist_pm_bg_pi_mom_mcs->Fill(momentum, ev.weight);
	  }
	  else if ( chain.stop_flag == 2 ) {
	    momentum = chain.bm_curvature_mom;
	    hist_pm_bg_pi_mom->Fill(momentum, ev.weight);
	    ...
	    // hist_pm_bg_pi_mom->Fill(momentum, ev.weight);
	    // hist_pm_bg_pi_mom_range->Fill(momentum, ev.weight);
	  }
	  hist_pm_bg_pi_cos->Fill(cosine, ev.weight);
	  hist_pm_bg_pi_deg->Fill(theta_deg, ev.weight);
	}
	else {
	  momentum = chain.ecc_mcs_mom[0];
	  hist_pm_bg_other_mom_mcs->Fill(momentum, ev.weight);
	  hist_pm_bg_other_cos->Fill(cosine, ev.weight);
	  hist_pm_bg_other_deg->Fill(cosine, ev.weight);
	}
      }
    }
  ...
  hist_pm_bg_mu_mom->Write();
  hist_pm_bg_mu_mom_mcs->Write();
  hist_pm_bg_mu_mom_range->Write();
  hist_pm_bg_mu_cos->Write();
  hist_pm_bg_mu_deg->Write();

  hist_pm_bg_mu_mom_mcs_all->Write();

  hist_pm_bg_mu_mom_true->Write();
  hist_pm_bg_mu_mom_true_stop->Write();
  hist_pm_bg_mu_mom_true_nonstop->Write();
  
  hist_pm_bg_pi_mom->Write();
  hist_pm_bg_pi_mom_mcs->Write();
  hist_pm_bg_pi_mom_range->Write();
  hist_pm_bg_p_mom->Write();
  hist_pm_bg_p_mom_mcs->Write();
  hist_pm_bg_p_mom_range->Write();
  hist_pm_bg_pi_cos->Write();
  hist_pm_bg_pi_deg->Write();
  hist_pm_bg_p_cos->Write();
  hist_pm_bg_p_deg->Write();
  hist_pm_bg_other_mom_mcs->Write();
  hist_pm_bg_other_cos->Write();
  hist_pm_bg_other_deg->Write();
